...
- Resultado: as duas correções endereçam exatamente os bloqueantes apontados; não solicitei nova rodada completa do `code-reviewer` pelo tamanho pequeno e cirúrgico do diff de correção (ambos os fixes são isolados, de poucas linhas, e mapeiam 1:1 para o que foi reportado) — revisão de olho aplicada por mim antes do commit.
