...
- [x] `fe-01` Markup do novo modal "Nova tarefa" (overlay + `.modal`, reaproveitando a classe `.modal` real do app — **360px/`border-radius:18px`, não os 420px/16px do protótipo**, ver correção do `design-critic` em "Decisões de UX tomadas"): campos Nome, Data, Link e footer com "Cancelar"/"Salvar tarefa"
       ...
       Depende de: nenhuma
- [x] `fe-02` Markup do toggle "Recorrente" + painel de recorrência (abas Diário/Semanal/Mensal/Personalizado, weekday-pills, campo numérico do personalizado, "Repetir até", linha de resumo textual) dentro do modal de `fe-01`
...
- [x] `fe-04` Barra "🔁 Série: {nome} · {resumo} até {data}" no topo do modal de edição existente, oculta por padrão
       ...
       Depende de: nenhuma
- [x] `fe-05` Markup dos dois modais de confirmação de escopo — "Editar tarefa recorrente" (✏️) e "Excluir tarefa recorrente" (🗑️), cada um com as duas opções em formato cartão-botão (a segunda opção de exclusão em cor de risco)
...
- **`fe-14` — `f.date`/`f.delegatedDate` não passam pela pergunta de escopo**: a task `fe-14` fala em interceptar "qualquer handler `f.*.addEventListener`" na primeira mudança de campo, mas os campos que alteram a data da tarefa (`f.date`, `f.delegatedDate`) são regidos por uma regra diferente e mutuamente exclusiva (`fe-16`: mover sempre vira exceção, nunca propaga para a série). Decisão conservadora: criada uma função `directPatch()` (mutação direta, sem pergunta de escopo) usada só por esses dois campos; o gate de escopo (`patch()`) continua cobrindo todos os demais campos (nome, link, duração, prioridade, concluída, delegado/delegado-para, urgente, campos customizados, equipe).
- **`fe-14` — "Cancelar" no modal de escopo de edição**: como o app usa auto-save por campo (sem "desfazer" de valor de input), a task não especifica o que ocorre visualmente se o usuário cancelar a pergunta de escopo depois de já ter digitado algo no campo. Decisão conservadora: nada é salvo (a mutação pendente é descartada) e o modal de edição é repopulado a partir do estado em memória (`openModal()` de novo), revertendo o valor visual do campo ao que já estava salvo. A pergunta volta a aparecer na próxima tentativa de edição da sessão.
- **`fe-11` — sem toggle "Urgente" no modal "Nova tarefa"**: a task `fe-11` menciona "se marcada urgente, aplica `urgentRankBase` decrescente por instância", mas nenhuma das tasks de estrutura (`fe-01`/`fe-02`, que listam os campos do modal como só Nome/Data/Link, espelhando o protótipo v2) inclui um controle de "Urgente" no modal de criação. Decisão conservadora: não adicionar um controle novo não previsto no design aprovado (v2 fechado) só para satisfazer essa frase da task; todas as instâncias criadas pelo modal "Nova tarefa" nascem com `urgent: false, urgentRank: 0` (mesmo default do `addTask()` atual). Quem quiser uma série urgente pode marcar as instâncias individualmente (ou usar "esta e todas as futuras" ao editar, `fe-14`) depois de criadas, pelo modal de edição existente, que já tem o campo Urgente.

### Problemas encontrados
- **[CRÍTICO — requer verificação manual do usuário] Bug de wipe de dados encontrado e corrigido durante a implementação, envolvendo o banco de PRODUÇÃO.** Commit `333af65` ("fix: evitar wipe de dados quando boards vem vazio"), aplicado em `server.js` antes da task `db-01`. O código antigo de `saveState()` tinha um fallback que, quando o payload chegava com `boards` vazio, executava `DELETE` sem filtro (`.delete().neq('id','')`) em `boards`, `tasks`, `calendar_events` e `people` — ou seja, um payload malformado ou um estado ainda não carregado no frontend apagaria todos os dados reais do usuário. O `.env` deste projeto aponta para o Supabase de **produção** (`cmppxsdtpekamafnbvkq.supabase.co`, o mesmo documentado no `CLAUDE.md`), não para uma instância de teste — não há ambiente de staging separado.
  - Dois arquivos `boards_import.csv` e `tasks_import.csv` apareceram na raiz do repo (não rastreados pelo git, sem histórico — nunca foram commitados), com dados que parecem reais (boards "Trabalho"/"Pessoal", nomes de tarefas específicos do usuário, 49 linhas de tasks). Tudo indica que foram um export manual de segurança feito em algum momento da implementação, antes ou depois de o bug ser corrigido — mas não há registro de quem gerou, quando, nem se houve de fato um wipe seguido de restauração, ou se foi só uma precaução preventiva.
  - **Eu (orquestrador) tentei verificar o estado atual do banco de produção rodando `node server.js` localmente e chamando `GET /api/tasks`, mas recebi `{"error":"JWT issued at future"}` do Supabase — não consegui confirmar programaticamente, a partir deste ambiente, se os dados de produção estão intactos.** Isso pode ser uma limitação do ambiente isolado onde rodei a verificação (sandbox), não necessariamente um problema com a chave em si — mas não dá para eu descartar a hipótese sem acesso real.
  - **Ação necessária do usuário antes de mergear esta feature:** abrir o app real (local ou no Render) e confirmar visualmente que os boards "Trabalho" e "Pessoal" e as tarefas neles continuam intactos, comparando com `boards_import.csv`/`tasks_import.csv` se precisar. Os dois CSVs foram adicionados ao `.gitignore` (não devem ser commitados — contêm dados pessoais) mas foram mantidos em disco como rede de segurança; podem ser apagados depois de confirmado que os dados estão OK.
...
**2026-07-12 (continuação — code-reviewer + correções)**
- Onde parei: `code-reviewer` revisou o diff completo (`schema.sql`, `server.js`, `public/app.js`, `public/index.html`, `public/styles.css`) e reprovou por 2 bloqueantes reais de corretude:
  1. **"Apenas esta ocorrência" nunca marcava `isException = true`** (`resolveEditScope`, `app.js:1228`) — a instância editada individualmente continuava contando como parte ativa da série, então um "esta e todas as futuras" aplicado depois a partir de uma instância anterior sobrescrevia silenciosamente a decisão do usuário de isolar aquela ocorrência (o filtro `!x.isException` em `applyPatchWithScope` não a excluía, porque o campo nunca tinha sido setado). **Corrigido:** `resolveEditScope` agora seta `t.isException = true` quando `choice === 'only'`, antes de aplicar o patch.
  2. **Criar recorrência sem escolher data de término falhava silenciosamente** (`updateRecurrenceSummary`, `app.js:837`) — o cálculo de `hasZeroOccurrences` só rodava se `rule.endDate` já tivesse valor; sem data de término, o resumo ficava no estado normal (sem erro) e o botão "Salvar tarefa" ficava habilitado, mas ao clicar `generateRecurrenceInstances` retornava `[]` e `handleSaveCreateTask` só dava `return` sem nenhum feedback. **Corrigido:** `hasZeroOccurrences` agora também é `true` quando `!rule.endDate`, reaproveitando o mesmo estado de erro inline já aprovado no v2 (sem criar uma mensagem nova).
- Também apliquei 2 das melhorias não-bloqueantes sugeridas, por serem baratas e diretamente relacionadas à corretude: (a) a barra "🔁 Série: ..." no modal de edição (`openModal`, `app.js:1167`) agora só aparece quando `!t.isException` — antes aparecia mesmo em instâncias já-exceção, sugerindo (incorretamente) que editar perguntaria escopo; (b) `#ct-name` recebeu o atributo `required`, consistente com o `add-form` existente. As demais sugestões (convergência futura de `.btn-primary`/`.btn-neutral` com os botões já existentes, `urgent_rank` como `INTEGER`) ficam registradas mas fora do escopo desta rodada.
- `node --check` confirmado em `app.js`/`server.js` após as correções.
- Rodei o `code-reviewer` de novo, focado só em confirmar as 2 correções (não uma revisão completa do zero, já que o diff de correção é pequeno e cirúrgico — poucas linhas, mapeando 1:1 para os bloqueantes reportados). Resultado: **aprovado.** Confirmou por evidência de código que (a) `isException=true` realmente tira a instância do filtro `!x.isException` usado pela edição em massa, e que reabrir a tarefa não pergunta mais escopo; (b) data de término ausente agora ativa o mesmo estado de erro/disabled, sem alterar o comportamento do caso normal (curto-circuito do `||` preserva o ramo já existente). Observação registrada (não bloqueante): o atributo `required` em `#ct-name` é inócuo mas não tem efeito prático, já que o modal não é um `<form>` e o botão "Salvar tarefa" não dispara `submit` — a proteção real contra nome vazio continua sendo o `if (!name) return;` em `handleSaveCreateTask`, que já existia.
- **Resultado final: branch aprovada para Pull Request.**
- Próximo passo: push da branch e abertura do PR. **Pendência que segue para o usuário, independente do código:** confirmar manualmente os critérios 13/14 (testado nos dois boards, persistência pós-reload) no app real, e verificar que os dados de produção não foram afetados pelo bug de wipe encontrado (`333af65`) — nenhum dos dois é verificável a partir deste ambiente de desenvolvimento.
