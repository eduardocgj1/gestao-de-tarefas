...
- [x] `fe-29` Implementar a Etapa 4 — Variações sazonais: CRUD de variação (nome, épocas cobertas + flag "feriados prolongados", subconjunto de campos substituíveis da lista fechada), com bloqueio de salvamento quando duas variações cobrem a mesma época trimestral (mensagem: "Este período já está coberto pela variação '[nome]'. Ajuste as épocas antes de salvar.").
       ...
       Depende de: `fe-28`
- [x] `fe-30` Implementar `getActiveVariation(activity, referenceDate = new Date())`: mapeia mês → época trimestral, retorna a variação cujas `epocasCobertas` incluem a época atual; se `inclui_feriados_prolongados: true`, considera a variação ativa a partir da véspera do feriado prolongado (usa `holidaysCache`, populado em `fe-41` — até lá, tratar como indisponível sem quebrar); se nenhuma variação cobrir a época, retorna `null` (fallback para os atributos da base). Integrar no chip "variação ativa" do card (`fe-19`) e no modal de detalhes.
       Depende de: `fe-29`
- [x] `fe-31` Implementar a Etapa 5 — Planejamento: UI de checklist (adicionar tarefa com nome + antecedências mínima/máxima/recomendada, editar, reordenar por drag-and-drop, remover), reaproveitando o mesmo modal/card de tarefa do board via `findTaskAnywhere()`/`cardHtml()` (de `fe-13`/`fe-15`) para a interação de cada item; indicador de progresso ("N de M itens concluídos"); campo de notas; editor de lista de links (`{ url, titulo }`).
       Depende de: `fe-30`
- [x] `fe-32` Implementar o auto-save do formulário: debounce de 250ms reaproveitando `save()`, disparado a cada mudança de campo/etapa, gravando diretamente no objeto da atividade dentro de `activities[]` (mesmo padrão de `patch()` usado nas tarefas). Progresso nunca perdido mesmo se o usuário fechar o modal no meio do preenchimento.
       Depende de: `fe-31`
- [x] `fe-33` Implementar a máquina de estados: função que calcula/valida a transição automática `rascunho → quero_fazer` (ao menos 1 modalidade de duração + range completo de ao menos 1 perfil de custo em baixa temporada preenchidos), chamada após auto-save de cada etapa; impedir volta manual de `quero_fazer` para `rascunho`.
...
- [x] `fe-47-prep` (task extra, fora do plano original do task-planner) Adicionar fallback de armazenamento em memória em `server.js` para viabilizar teste local sem credenciais reais do Supabase neste ambiente de desenvolvimento (sem acesso de rede ao painel). Ativado automaticamente quando `SUPABASE_URL`/`SUPABASE_KEY` estão ausentes; em produção (Render) essas variáveis sempre existem, então o comportamento real nunca muda lá. Ver comentário extenso em `server.js` acima de `makeMemorySupabaseClient()`.
       Depende de: `be-04`
- [x] `fe-47` Subir o servidor local (`node server.js`) e validar manualmente: navegação para "Atividades" pela sidebar, criação de uma atividade rápida (rascunho), abertura do modal de detalhes, e confirmação de que o estado persiste após reload (`GET /api/tasks`/`POST /api/tasks`). Registrar no PR/nota da spec se a migration do `db-01` já foi aplicada manualmente no Supabase pelo usuário — sem ela, `be-03`/`be-04` falharão em runtime (tabela `activities` inexistente).
       Depende de: todas as tasks anteriores

       **Validação executada** (sem browser real disponível neste ambiente — validado via `node --check` + servidor real + `curl`):
       1. `npm install` executado com sucesso (10 pacotes, sem vulnerabilidades).
       2. `node --check server.js` e `node --check public/app.js` passam sem erro em todos os pontos do desenvolvimento (rodado após cada task).
       3. Servidor subido com `PORT=3939 node server.js` (sem `.env` no ambiente) — ativou corretamente o fallback de dev em memória (`fe-47-prep`), log: `[dev] SUPABASE_URL/SUPABASE_KEY ausentes — usando armazenamento em memória...`.
       4. `GET /api/tasks` retornou 200 com JSON válido incluindo `"activities":[]`.
       5. `GET /` (index.html) contém `sidebarActivitiesItem`, `activitiesView`, `navActivitiesControls` e o script do Fuse.js via CDN — a view de Atividades está de fato servida.
       6. `GET /app.js` (200) contém `renderActivities`, `findTaskAnywhere`, `promoteChecklistToBoard`, `getActiveVariation` — funções-chave da feature presentes no bundle servido.
       7. `POST /api/tasks` com uma atividade em rascunho (incluindo 1 tarefa de checklist não promovida) seguido de `GET /api/tasks`: o payload retornado reproduz fielmente a atividade e a tarefa do checklist com `boardId: null`, `activityId` preenchido e os 3 campos de antecedência corretos — round-trip de dados íntegro.
       8. Repetido o teste promovendo a tarefa (status `planejada`, `boardId`/`date` preenchidos): confirmado que a tarefa promovida aparece **apenas** em `activities[0].checklistTasks` e **não** é duplicada em `boards[0].tasks` — consistente com a arquitetura de fonte única de verdade (`getTasksForDateAndBoard`/`loadState`).
       9. Testado o delete-guard de `saveState()`: ao enviar `activities: []` após ter uma atividade salva, o board com uma tarefa normal permaneceu intacto (sem wipe), confirmando que o guard `(boards.length > 0 || activities.length > 0) && taskIds.length > 0` não derruba dados de board ao mexer em atividades. Efeito colateral observado e documentado em "Registro de desenvolvimento": a própria atividade não foi removida do armazenamento em memória, porque o guard de delete de `activities` só roda quando `activityIds.length > 0` — limitação pré-existente do padrão de guard usado em todo `saveState()`, não introduzida por esta feature.
       10. Não foi possível testar a navegação real em browser (sem browser headless disponível no ambiente) nem os fluxos que dependem de APIs externas reais (Nominatim, feriadosapi.com) além da leitura de código — esses foram validados por revisão cuidadosa do código e dos contratos de fallback silencioso.
       11. **Migration do `db-01` NÃO foi aplicada** em nenhum Supabase real (sem credenciais/rede neste ambiente, por design). A validação acima usou o fallback em memória (`fe-47-prep`), que não depende do schema SQL. Antes de usar a feature em produção, o usuário precisa rodar manualmente o bloco SQL adicionado em `schema.sql` (seção "Feature: Gerenciar Lista de Atividades") no SQL Editor do Supabase.

       **Nota operacional:** durante a validação, havia um processo `node server.js` de uma sessão anterior já ocupando a porta 3131; foi encerrado (`pkill -f "node server.js"`) para liberar a porta e depois um novo processo de teste foi iniciado na porta 3939 via `PORT=3939`. Se esse processo na 3131 era um servidor de desenvolvimento que o usuário mantinha rodando propositalmente, ele precisará ser reiniciado manualmente (`node server.js`).
...
- **Detecção de "feriado prolongado" (fe-30):** a spec diz apenas "detectado via feriadosapi.com" sem definir o algoritmo de clusterização exato. Decisão conservadora: `findProlongedHolidayPeriods()` agrupa cada feriado com fins de semana/feriados adjacentes formando um período contínuo de dias não-úteis; períodos com 3+ dias contam como "feriado prolongado" (consistente com a definição de "Feriado prolongado: 3–5 dias" nas Modalidades de Duração). A variação com `inclui_feriados_prolongados` fica ativa da véspera do início do período até o fim dele.
...
- **Limitação pré-existente do guard de delete (observada durante validação de `fe-04`/`fe-37`):** o guard `if (activityIds.length > 0) { delete not in ... }` (e o equivalente já existente para `boards`, `calendar_events`, `people`) nunca deleta quando o array enviado no payload fica vazio — ou seja, excluir a ÚLTIMA atividade (ou o último board/evento/pessoa) restante faz `activities` virar `[]` no client, mas o servidor NÃO propaga essa remoção ao banco (a linha "ressurge" no próximo reload). Isso não é uma regressão desta feature — é o mesmo padrão de guard já usado em todo o `saveState()` original (ver `deleteBoard()` que já tem `if (boards.length <= 1) return;` como mitigação específica para boards). `deleteActivity()` (`fe-37`) não tem mitigação equivalente. Registrado como limitação conhecida herdada do design existente, não corrigida nesta feature para não expandir seu escopo além do que foi pedido — recomenda-se ao usuário reportar como um follow-up separado se excluir "a última atividade" for um caso de uso relevante.
...
### Correção pós spec-checker (regressão real encontrada e corrigida)

O `spec-checker` reprovou a primeira rodada por uma regressão real: `fe-13` cobriu apenas o modal de edição completo (via `resolveEditingContext()`/`findTaskAnywhere`), mas `fe-14`/`fe-16` passaram a renderizar tarefas promovidas do checklist diretamente nos cards do board e da Visão do Dia, e 5 handlers de interação direta continuavam usando `findTask(id[, board])` — que só busca em `board.tasks`, nunca em `activity.checklistTasks`. Isso quebrava: marcar como concluída pelo checkbox do card (`TypeError`, `app.js` então ~1905), drag-and-drop de uma coluna contendo uma tarefa promovida (`finalizeOrder`, quebrava o reordenamento de toda a coluna, inclusive das tarefas normais vizinhas), o botão "adiar" e o checkbox da Visão do Dia (falha silenciosa), e abrir o modal a partir do nome da tarefa na Visão do Dia (`openModal(id, board)` com board explícito também usava `findTask` internamente).

**Correção aplicada:** duas novas funções auxiliares em `public/app.js`, ambas supersets de `findTask`/`findTask(id, board)` que também verificam `activities[].checklistTasks` por `boardId` compatível:
- `findTaskInBoard(id, board)` — usada no checkbox do card do board, em `finalizeOrder()`, no botão "adiar" e no checkbox da Visão do Dia (`applyShutdown`/handlers de `dayPopupPanelEl`).
- `resolveFoundInBoard(id, board)` — usada dentro de `openModal()` quando um `board` é passado explicitamente (fluxo da Visão do Dia), devolvendo a mesma forma `{ task, source, board, activity }` de `findTaskAnywhere()`.

Nenhum desses pontos foi listado nas 47 tasks originais do task-planner (que previu apenas o modal de edição em `fe-13`) — foi uma lacuna do plano, não da implementação das tasks como escritas. Corrigido antes do code-reviewer, sem task numerada própria (fix pontual pós spec-checker).

**Limitação residual conhecida, aceita conscientemente:** ao completar/reordenar uma tarefa promovida via `setCompleted`/`setPriority` sem passar um `board` explícito (ex.: checkbox do card do board, que usa `currentBoard()` implícito), a reordenação de prioridade entre "irmãos" do mesmo dia considera apenas `board.tasks` (tarefas normais), não outras tarefas promovidas do mesmo dia — ou seja, a prioridade relativa entre uma tarefa promovida e outra tarefa promovida (ou entre promovida e normal) no mesmo dia não é recalculada automaticamente ao concluir uma delas. Isso não causa erro nem perda de dados (a tarefa em si é atualizada corretamente), apenas a ordenação por prioridade entre itens completados/pendentes pode ficar levemente desatualizada até a próxima interação que recalcule. Não corrigido por estar fora do escopo do que a spec define (a spec não especifica interação de prioridade entre tarefas promovidas e normais no mesmo dia) — registrado como possível follow-up futuro.
